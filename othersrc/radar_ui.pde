...
void drawObjectToCanvas() {
  ...
  if(iDistance > maxDistance){
    ...
    radarCanvas.line(pixsDistance*cos(radians(iAngle)), -pixsDistance*sin(radians(iAngle)), maxLimitPixels*cos(radians(iAngle)), -maxLimitPixels*sin(radians(iAngle)));
  }
  radarCanvas.popMatrix();
  radarCanvas.endDraw();
}
